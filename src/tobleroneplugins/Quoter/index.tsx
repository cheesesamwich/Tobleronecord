...
async function createQuoteImage(avatarUrl: string, quoteOld: string, grayScale: boolean): Promise<Blob> {
    ...
    switch(setStyle)
    {
        ...
        case ImageStyle.inspirationalSquare:
            ...
            const avatarSquareBlob = await fetchImageAsBlob(avatarUrl);
            const fadeSquareBlob = await fetchImageAsBlob("https://files.catbox.moe/54e96l.png");

            const avatarSquare = new Image();
            const fadeSquare = new Image();

            const avatarSquarePromise = new Promise<void>(resolve => {
                avatarSquare.onload = () => resolve();
                avatarSquare.src = URL.createObjectURL(avatarSquareBlob);
            });

            const fadeSquarePromise = new Promise<void>(resolve => {
                fadeSquare.onload = () => resolve();
                fadeSquare.src = URL.createObjectURL(fadeSquareBlob);
            });

            await Promise.all([avatarSquarePromise, fadeSquarePromise]);

            ctx.drawImage(avatarSquare, 0, 0, squareSize, squareSize);
            ...
            ctx.drawImage(fadeSquare, squareSize - 400, 0, 400, squareSize);

            ctx.fillStyle = "#fff";
            ctx.font = "italic 36px Georgia";  // Increased font size
            const squareQuoteWidth = squareSize - 50;
            const squareQuoteX = 25;
            const squareQuoteY = squareSize / 2 - 50;
            wrapText(ctx, `"${quote}"`, squareQuoteX, squareQuoteY, squareQuoteWidth, 36, preparingSentence, lines);  // Adjusted line height

            const squareWrappedTextHeight = lines.length * 45;

            ctx.font = "bold 30px Georgia";  // Increased font size
            const squareAuthorNameX = (squareSize / 2) - (ctx.measureText(`- ${name}`).width / 2);
            const squareAuthorNameY = squareQuoteY + squareWrappedTextHeight + 40;

            ctx.fillText(`- ${name}`, squareAuthorNameX, squareAuthorNameY);
            preparingSentence.length = 0;
            lines.length = 0;
            return await canvasToBlob(canvas);
    }
